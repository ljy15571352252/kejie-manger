var DateUtils = {
  getInterval: function (_date, targetDate, target) {// 固定日期和当前日期的比较计算时差天数，例如：getInterval("2016-09-23")
    var date1 = _date.getTime(), date2 = targetDate.getTime()
    switch (target) {
      case 'y':
        return Math.ceil((date1 - date2) / (24 * 3600 * 1000*365))
      case 'd'://计算两个时间相差的天数
        return Math.ceil((date1 - date2) / (24 * 3600 * 1000))
      case 'm'://计算相差的分钟数
        var cacl = _date - targetDate
        var leave1 = cacl % (24 * 3600 * 1000)
        var minutes = Math.ceil(leave1 / (60 * 1000))
        return minutes
      case 'h':
        var cacl = _date - targetDate
        if (cacl < 0) return 0
