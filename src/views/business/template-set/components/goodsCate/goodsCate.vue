<template>
  ...
  >
    ...
    <div class="diy-module">
      <img
        src="http://jiadehao.oss-cn-shenzhen.aliyuncs.com/temp/e150c678a831adc1a84121b37666cbfd.png"
        alt=""
        @click="enableTem(item.enable,item.code)"
      />
      <div class="preview-module">
        ...
        <Button
          icon="md-eye"
          @click="getImagePreview('http://jiadehao.oss-cn-shenzhen.aliyuncs.com/temp/e150c678a831adc1a84121b37666cbfd.png')"
        >大图预览</Button>
        <!-- <Button
          icon="md-settings"
          @click="useTem(item.code)"
...
<script>
import { getCustTemplateList, enableCustTemplate } from "@/api/store/store";
export default {
    name: "index",
    data() {
        return {
    ...
    methods: {
        ...
        addModule() {
            this.$router.push({
                path: "/diyModule/add"
                // query: {
                //     code: this.custIndexList[0].code
                // }
            });
        },
        getImagePreview(url) {
