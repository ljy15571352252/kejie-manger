    ...
    <div class="pageContent flexC form-content">
      ...
      >
        ...
        <Alert
          class="info"
          type="info"
        >图片格式为 JPG/JPEG/PNG/GIF 大小在2MB内,点击图片重新上传</Alert>
        <FormItem
          label="二维码："
          prop="qrcode"
          ...
          <image-upload @on-upload-success="
            ...
            >
              <img
                :src="myForm.qrcode"
                v-if="myForm.qrcode"
                @click="$set(myForm, 'qrcode', '')"
              />
              <span v-if="!myForm.qrcode">+</span>
            </div>
          </image-upload>
        </FormItem>
        <!-- <FormItem
...
<script>
import {
    getMatchmmakerConfig,
    updateMatchmmakertConfig
...
export default {
    ...
    async created() {
        let res = await getMatchmmakerConfig({
            type: 2
        });
        this.myForm = res;
    },
