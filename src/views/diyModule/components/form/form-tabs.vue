...
export default {
    ...
    methods: {
        ...
        handleDragDrop(a, b) {
            ...
            if (this.formDynamic.items[0].name != "推荐") {
                this.isShowNavigator(false);
            } else if (this.formDynamic.items[0].name == "推荐") {
        ...
        handleTabsAdd() {
            if (this.formDynamic.items.length < 4) {
                this.showModal = true;
                this.isEdit = false;
                this.myForm = {};
            } else {
        ...
        beforeHandleTabRemove(idx) {
            if (this.formDynamic.items.length == 2) {
                this.$Message.error("至少存在两个！");
                return Promise.reject();
            } else if (this.formDynamic.items[idx].name == "推荐") {
