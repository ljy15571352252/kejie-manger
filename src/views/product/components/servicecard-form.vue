        ...
        >
            <div>
                <Row>
                    ...
                    <Col :span="span" class="form-item-padding">
                        <FormItem label="有效天数" prop="validNum">
                            <InputNumber
                                style="width: 240px"
                                v-model="myForm.validNum"
                                :disabled="mode === PAGE_MODE.DETAIL"
                            ></InputNumber>
                        </FormItem>
                    </Col>
              ...
              <Row>
                <Col :span="span" class="form-item-padding">
                  <FormItem label="使用次数" prop="price">
                    <InputNumber
                        style="width: 240px"
                        v-model="myForm.count"
                        :disabled="mode === PAGE_MODE.DETAIL"
                    ></InputNumber>
                  </FormItem>
                </Col>
                <Col :span="span" class="form-item-padding">
                  <FormItem label="状态" prop="state">
                    <RadioGroup v-model="myForm.state">
                      <Radio :label="item.value"  v-for="(item,index) in upOrDownStatusOptions" :key="index">{{item.label}}</Radio>
                    </RadioGroup>
                  </FormItem>
                </Col>
              </Row>
...
import {addServiceCard,editServiceCard} from "@/api/product/servicecard";
import {amountFormat, amountParser, upOrDownStatusOptions} from "@/common/utils/viewui-utils";

export default {
    name: "serviceCard-form",
    ...
    methods: {
        onReset() {
            ...
            if (this.mode !== PAGE_MODE.ADD) {
                this.myForm = this.serviceCardObj;
            }
        },
        async onSubmit() {
