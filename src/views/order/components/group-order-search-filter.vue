        ...
        <Row :gutter="20">
            ...
            <Col :span="span">
                <FormItem label="订单状态" prop="state" :label-width="80">
                    ...
                    >
                        <Option
                            v-for="item in groupOrderStateOptions"
                            :value="item.value"
                            :key="item.value"
                            >{{ item.label }}
                        </Option>
...
import {
    DatePickerOptions,
    groupOrderStateOptions,
} from "@/common/utils/viewui-utils";

export default {
    name: "group-order-search-filter",
    data() {
        return {
            searchPlaceHolder: "请输入群组名称",
            span: 6,
            dateOptions: DatePickerOptions,
            groupOrderStateOptions: groupOrderStateOptions(),
